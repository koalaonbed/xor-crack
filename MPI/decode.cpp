...
char* guess_keys(char most_char)
{
    ...
    {
        if(i + half*MPIrank >= key_len)
            break;

        int maxc = 0;
        //int* chars_count;
        int *chars_count =  chars_count_at_offset(key_len, i + half*MPIrank);
        ...
        for(int j = 0; j < C_CHAR_MAX; j++)
        {
            if(maxc == chars_count[j])
            {
                tmp[i][num] = (char) (j ^ (int) most_char);
                num++;
            }
        }
    }
...
void string_generator_OuO(char tmp[][10], int key_no, int *magicshit, int half)
{
    ...
    {
        int server = 0;
        for(int i = 0, k = 0; i < most_poss_key_len; i++, k++)
        {
            if(k == half)
            {
                k = 0;
                server++;
                MPI_Status status;
                MPI_Recv(tmp, half*10, MPI_CHAR, server, 99, MPI_COMM_WORLD, &status);
                MPI_Recv(magicshit, half, MPI_CHAR, server, 95, MPI_COMM_WORLD, &status);
            }
            for(int j = 0; j < 10; j++)
            {
                key_possible_bytes[i][j] = tmp[k][j];
            }
            shit[i] = magicshit[k];
        }
        for(int i = 0; i < most_poss_key_len; i++)
        {
            poss_key_set[key_no][i] = key_possible_bytes[i][shit[i]];
        }

    }
    else
    {
        MPI_Send(tmp, half*10, MPI_CHAR, 0, 99, MPI_COMM_WORLD);
        MPI_Send(magicshit, half, MPI_CHAR, 0, 95, MPI_COMM_WORLD);
    }

    MPI_Bcast(poss_key_set, 10000*MAX_KEY_LEN, MPI_CHAR, 0, MPI_COMM_WORLD);
    
}

void key_generator(char tmp[][10], int *magicshit, int offset, int half)
{
    
    if(offset < half-1)
        key_generator(tmp, magicshit, offset+1, half);
    else
    {
        string_generator_OuO(tmp, found_keys, magicshit, half);
        found_keys++;
    }
     
    for(int i = 1, j = magicshit[offset]; i <= j; i++)
    {
        if(offset + half*MPIrank >= most_poss_key_len)
            break;
...
char* all_keys(char tmp[][10], char* key_part, int offset)
{
    ...
    key_generator(tmp, magicshit, 0, half);
    /*
    for(int i = 0; i < most_poss_key_len; i++)
...
void produce_plaintexts()   
{
    ...
    for(int i = 0; i < found_keys; i++)
    {
        ...
        for(int j = half*MPIrank; j < half*(MPIrank+1); j++)
        {
            if(j >= fsize)
                break;
